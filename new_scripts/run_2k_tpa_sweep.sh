#!/bin/bash
# =============================================================================
# 2k-step Tensor Product Attention sweep — full pipeline (base → mid → SFT → eval)
#
# Probe run for TPA (Zhang et al., NeurIPS 2025, arXiv:2501.06425).
# The user reports that for d12 models on this setup, val bpb usually plateaus
# by ~2k steps — so 2k is the "is this worth a 5k commit?" decision point.
#
# Each row swaps in TPA's rank-factorized Q/K/V instead of MHA's three projections;
# the rest of the pipeline (Block, MLP, RoPE, QK-norm, EMA, JEPA, mid + SFT + CORE)
# is byte-identical to run_5k_pg_sweep.sh so val-bpb / CORE comparisons line up
# directly against the GPT, RYS, and PG-block rows there.
#
# 3 ROWS (in execution order):
#   1) tpa_gpt R_Q=6 R_K=2 R_V=2 + CE                  → SFT via chat_sft
#      "T6" — paper sweet spot. Q rank == n_head (full Q expressiveness),
#      KV rank 2 (heavy KV compression). Per-attn-layer params -31% vs MHA.
#   2) tpa_gpt R_Q=2 R_K=2 R_V=2 + CE                  → SFT via chat_sft
#      "T2" — aggressive, -49% per-layer attn params. Tests whether Q can
#      also be heavily rank-compressed at our scale.
#   3) tpa_gpt R_Q=6 R_K=2 R_V=2 + JEPA (linear)       → SFT via chat_sft_jepa
#      "T6 × JEPA-lin" — TPA crossed with the best JEPA config from
#      SWEEP_REPORT.md, factorial design.
#
# Usage:
#   tmux new -s tpa-2k
#   bash new_scripts/run_2k_tpa_sweep.sh
#
#   # Just the val-bpb / base-CORE predictor signal (skip mid + SFT + SFT eval):
#   SKIP_MID=1 SKIP_SFT=1 SKIP_SFT_EVAL=1 bash new_scripts/run_2k_tpa_sweep.sh
#
#   # Single row only (T6 first):
#   ROWS="1" bash new_scripts/run_2k_tpa_sweep.sh
#
#   # Pick up where a previous launch left off:
#   SKIP_EXISTING=1 bash new_scripts/run_2k_tpa_sweep.sh
#
# Skip phases (any combination, all default 0 = run):
#   SKIP_BASE=1     — skip 2k base training (assumes checkpoints exist)
...
#
# Tags written to:
#   ~/.cache/nanochat/last_tpa_sweep_base_tags.txt
#   ~/.cache/nanochat/last_tpa_sweep_sft_tags.txt
# =============================================================================
...
# --- Base training: shared budget (matches run_5k_pg_sweep.sh except num_iterations).
# Keeping every other knob identical so the 2k val-bpb / CORE signal is a clean
# subset of the 5k curve from the PG sweep — direct apples-to-apples comparison.
DEPTH="${DEPTH:-12}"
...
# --- TPA rank presets ---
TPA_T6_RQ="${TPA_T6_RQ:-6}"   # T6: Q rank == n_head, full Q expressiveness
TPA_T6_RK="${TPA_T6_RK:-2}"
TPA_T6_RV="${TPA_T6_RV:-2}"
TPA_T2_RQ="${TPA_T2_RQ:-2}"   # T2: aggressive Q compression too
TPA_T2_RK="${TPA_T2_RK:-2}"
TPA_T2_RV="${TPA_T2_RV:-2}"

# --- Mid-training defaults (mirror run_5k_pg_sweep.sh) ---
...
# --- SFT defaults (mirror run_5k_pg_sweep.sh) ---
...
# --- W&B ---
WANDB_PREFIX="${WANDB_PREFIX:-5k-tpa-sweep}"
WANDB_RUN="${WANDB_RUN:-}"
...
echo ""
echo "================================================================================"
echo "Tensor Product Attention 5k probe sweep — full pipeline (base → mid → SFT → eval)"
...
echo "  TPA:  T6 ranks=(Q=${TPA_T6_RQ}, K=${TPA_T6_RK}, V=${TPA_T6_RV})"
echo "        T2 ranks=(Q=${TPA_T2_RQ}, K=${TPA_T2_RK}, V=${TPA_T2_RV})"
...
# -----------------------------------------------------------------------------
# Row registry (mirrors PG sweep schema — keeps run_*_sweep.sh scripts uniform).
#   ROW_TAG[i]      → checkpoint tag (also model dir name)
#   ROW_ARCH[i]     → architecture: tpa_gpt
#   ROW_JLAM[i]     → base-training JEPA lambda (0 = pure CE)
#   ROW_JSCHED[i]   → base-training JEPA schedule
#   ROW_EXTRA[i]    → extra CLI args for base_train_jepa (TPA ranks)
...
ROW_TAG[1]="d12-s5k-tpa-T6-ce-4090"
ROW_ARCH[1]="tpa_gpt"
ROW_JLAM[1]="0"
ROW_JSCHED[1]="constant"
ROW_EXTRA[1]="--tpa-rank-q ${TPA_T6_RQ} --tpa-rank-k ${TPA_T6_RK} --tpa-rank-v ${TPA_T6_RV}"
ROW_IS_JEPA[1]="0"

ROW_TAG[2]="d12-s5k-tpa-T2-ce-4090"
ROW_ARCH[2]="tpa_gpt"
ROW_JLAM[2]="0"
ROW_JSCHED[2]="constant"
ROW_EXTRA[2]="--tpa-rank-q ${TPA_T2_RQ} --tpa-rank-k ${TPA_T2_RK} --tpa-rank-v ${TPA_T2_RV}"
ROW_IS_JEPA[2]="0"

ROW_TAG[3]="d12-s5k-tpa-T6-jepa-lin-4090"
ROW_ARCH[3]="tpa_gpt"
ROW_JLAM[3]="0.25"
ROW_JSCHED[3]="linear_decay"
ROW_EXTRA[3]="--tpa-rank-q ${TPA_T6_RQ} --tpa-rank-k ${TPA_T6_RK} --tpa-rank-v ${TPA_T6_RV}"
ROW_IS_JEPA[3]="1"

# Derive an SFT-checkpoint tag from a base tag + JEPA flag.
# (Mirrors the convention in run_5k_pg_sweep.sh: append `_jepa` for JEPA SFT runs.)
sft_tag_for() {
...
mkdir -p "${CACHE_DIR}"
BASE_TAGS_FILE="${CACHE_DIR}/last_tpa_sweep_base_tags.txt"
SFT_TAGS_FILE="${CACHE_DIR}/last_tpa_sweep_sft_tags.txt"
...
run_base_row() {
    ...
    # No TPA row sets --depth in extra; left for parity with the PG-sweep template.
    if [[ "${extra}" == *"--depth"* ]]; then
        dim_args=()
    fi
...
echo ""
echo "================================================================================"
echo "TPA 5k probe sweep finished."
